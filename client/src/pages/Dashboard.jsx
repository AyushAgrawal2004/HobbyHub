...
const ContestSection = ({ isAdmin }) => {
    ...
    if (contests.length === 0) return (
        <div className="glass-panel" style={{ padding: '40px', marginBottom: '40px', textAlign: 'center' }}>
            <h3>No Active Contests</h3>
            <p style={{ color: 'var(--text-secondary)', marginBottom: isAdmin ? '20px' : '0' }}>
                Check back later for new challenges and prizes!
            </p>
            {isAdmin && (
                <button onClick={() => setShowCreateModal(true)} className="btn btn-primary">Create First Contest</button>
            )}
